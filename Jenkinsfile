pipeline {
	...
	environment {
		DOCKER_IMAGE = "nastss6tx5/asset-spy-statistics-service"
		DOCKER_TAG = "${env.BUILD_NUMBER}"
		GITHUB_REPO = "Nastss6tx5/asset-spy-statistics-service"
				...
				stage('Deploy to Kubernetes') {
					steps {
						...
						withCredentials([file(credentialsId: KUBECONFIG_CREDENTIALS_ID, variable: 'KUBECONFIG')]) {
							sh '''
							kubectl -n asset-spy set image deploy/statistics-service statistics-service=${DOCKER_IMAGE}:${DOCKER_TAG}
							kubectl -n asset-spy rollout status deploy/statistics-service --timeout=300s
							'''
						}
					}
				}
